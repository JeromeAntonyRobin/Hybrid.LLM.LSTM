...
import datetime
import threading
import os  # For suppressing TF warnings, though not directly used for file ops in this version
...
class StockPredictorApp:
    def __init__(self, master):
        ...
        master.geometry("1000x800")  # Adjust size as tabs reduce horizontal clutter
        ...
        self.single_prediction_tab.grid_columnconfigure(0, weight=1)  # Left column for config+metrics
        self.single_prediction_tab.grid_columnconfigure(1, weight=2)  # Right column for plot
        self.single_prediction_tab.grid_rowconfigure(0, weight=1)  # Plot row
        ...
        self.left_panel_single.grid_rowconfigure(0, weight=0)  # Config frame fixed height
        self.left_panel_single.grid_rowconfigure(1, weight=1)  # Metrics frame fixed height (can make 0)
        ...
        self.metrics_frame.grid(row=1, column=0, padx=5, pady=5, sticky="nsew")  # Below config frame
        ...
        self.experiment_tab.grid_columnconfigure(0, weight=1)  # Controls
        self.experiment_tab.grid_columnconfigure(1, weight=2)  # Results Table
        self.experiment_tab.grid_rowconfigure(0, weight=0)  # Config frame
        self.experiment_tab.grid_rowconfigure(1, weight=1)  # Results table/plot
        ...
        ttk.Label(self.experiment_config_frame, text="Look Backs (comma-sep):").grid(row=0, column=0, padx=5, pady=2,
                                                                                     sticky="w")
        self.experiment_lookbacks_var = tk.StringVar(value="30,60,90")
        ttk.Entry(self.experiment_config_frame, textvariable=self.experiment_lookbacks_var).grid(row=0, column=1,
                                                                                                 padx=5, pady=2,
                                                                                                 sticky="ew")

        ttk.Label(self.experiment_config_frame, text="Number of Runs per LB:").grid(row=1, column=0, padx=5, pady=2,
                                                                                    sticky="w")
        self.num_experiment_runs_var = tk.IntVar(value=5)
        ttk.Entry(self.experiment_config_frame, textvariable=self.num_experiment_runs_var).grid(row=1, column=1, padx=5,
                                                                                                pady=2, sticky="ew")

        self.run_experiment_button = ttk.Button(self.experiment_config_frame, text="Run Experiment",
        ...
        self.experiment_results_display_frame.grid(row=1, column=0, columnspan=2, padx=5, pady=5,
                                                   sticky="nsew")  # Span both columns
        self.experiment_results_display_frame.grid_columnconfigure(0, weight=1)
        self.experiment_results_display_frame.grid_rowconfigure(0, weight=1)  # Treeview
        self.experiment_results_display_frame.grid_rowconfigure(1, weight=1)  # Plot

        self.exp_results_tree = ttk.Treeview(self.experiment_results_display_frame,
                                             columns=("Look Back", "Avg MSE", "Avg R2", "Avg MAPE", "Avg Accuracy"),
                                             show="headings")
        ...
        tree_scrollbar = ttk.Scrollbar(self.experiment_results_display_frame, orient="vertical",
    ...
    def _run_single_prediction_thread(self):
        ...
        self.experiment_status_label.config(text="")  # Clear experiment status
    ...
    def _run_experiment_thread(self):
        self.run_button.config(state=tk.DISABLED)
        self.run_experiment_button.config(state=tk.DISABLED)
        self.status_label.config(text="")  # Clear single prediction status
    ...
    def _perform_experiment_task(self):
        try:
            ...
            try:
                ...
                raise ValueError(
                    "Invalid look back values for experiment. Enter comma-separated integers (e.g., '30,60,90').")

            if num_runs <= 0:
                raise ValueError("Number of runs per look back must be a positive integer.")

            all_experiment_averages = []  # To store averaged metrics for each look_back
            ...
            for i, current_look_back in enumerate(look_backs):
                individual_run_metrics = []  # Store raw metrics for this specific look_back across runs
                for run_idx in range(num_runs):
                    self.experiment_status_label.config(
                        text=f"Experiment: LB {current_look_back}, Run {run_idx + 1}/{num_runs}. Model: {current_model_name}")
                    self.master.update_idletasks()
                    ...
                    try:
                        metrics, _ = self._run_core_prediction_logic(
                            ...
                            look_back=current_look_back,  # Use the experiment's look_back
                            train_test_split_ratio=current_train_ratio,
                            selected_model_name=current_model_name
                        )
                        # We need the raw float values for averaging
                        ...
                        # Optionally, add a placeholder or skip this run if it failed
                        messagebox.showwarning("Experiment Run Error",
                                               f"Failed for Look Back {current_look_back}, Run {run_idx + 1}: {e}. Continuing experiment.")
                        # Do not append results for failed runs

                if individual_run_metrics:  # Only average if there were successful runs
            ...
            # Sort results by look back for better plotting
            ...
            if all_experiment_averages:
                ...
                # Create a twin Axes for R2 if you want to plot both on different scales
