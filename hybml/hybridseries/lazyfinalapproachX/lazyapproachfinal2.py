...
class ForwardLookingStockPredictor:
    ...
    def __init__(self, master):
        ...
        tk.Label(self.input_frame, text="Choose Sentiment Analyzer:").grid(row=4, column=0, padx=5, pady=5, sticky="w")
        ...
        self.analyzer_choice.grid(row=4, column=1, padx=5, pady=5, sticky="ew")

        self.predict_button = tk.Button(self.input_frame, text="Predict Next Day Price & Plot",
                                        command=self._start_prediction_thread)
        self.predict_button.grid(row=5, column=0, columnspan=2, pady=10, sticky="ew")

        self.ollama_status_label = tk.Label(self.input_frame, text="Ollama Status: Checking...", fg="gray")
        self.ollama_status_label.grid(row=6, column=0, columnspan=2, pady=5, sticky="ew")
    ...
    def _start_prediction_thread(self):
        ...
        lookback_window = self.lookback_entry.get()
        selected_analyzer = self.analyzer_choice.get()

        if not all([ticker, start_date_str, end_date_str, lookback_window, selected_analyzer]):
        ...
        try:
            ...
            messagebox.showwarning("Input Error", "Lookback window must be an integer. Dates must be APAC-MM-DD.")
            return
        ...
        thread = threading.Thread(target=self._perform_prediction_task,
                                  args=(ticker, start_date_str, end_date_str, int(lookback_window), selected_analyzer))
        thread.daemon = True
        thread.start()

    def _perform_prediction_task(self, ticker, start_date_str, end_date_str, lookback_window, selected_analyzer):
        """
        ...
        try:
            ...
            # Create a new list of column names by flattening MultiIndex tuples
            # E.g., ('Close', 'AAPL') becomes 'Close'
            flattened_columns = []
            for col in stock_data.columns:
                if isinstance(col, tuple):
                    # Take the first element for common stock metrics (e.g., 'Close' from ('Close', 'AAPL'))
                    # This assumes the first element is the descriptive part of the metric.
                    flattened_columns.append(col[0])
                else:
                    # For non-tuple columns, use them as is.
            ...
            # Yfinance news typically provides news for the last ~2 days.
            self._update_status(
            ...
            # --- Step 6: Apply Sentiment-Based Adjustment ---
            last_actual_price = stock_price_df['Close'].iloc[-1]
            # prediction_date is a Timestamp object from the index
            ...
            # Convert to standard datetime object for safe formatting and plotting
            ...
            if latest_sentiment_score >= sentiment_positive_threshold:
                ...
                sentiment_influence_msg = f"Neutral News ({selected_analyzer} score: {latest_sentiment_score:.2f}). Prediction relies more on historical trend."
                # No additional adjustment for neutral sentiment
            ...
            prediction_text = (
                ...
                f"Influence of Latest News: {sentiment_influence_msg}\n"
            ...
            self.master.after(0, lambda: messagebox.showinfo("Prediction Complete",
                                                             "Next day stock price predicted with news sentiment influence!"))
            ...
            # Pass the standard datetime object for plotting
            self._plot_prediction_and_history(ticker, stock_price_df, prediction_date_for_display,
    ...
    def _plot_prediction_and_history(self, ticker, historical_df, predicted_date, predicted_price):
        ...
        self.ax.plot(plot_dates, plot_prices, 'ro--', label='Predicted Next Day Price',
                     markersize=8)  # Red circle for prediction
