...
class ForwardLookingStockPredictor:
    """
    A Tkinter application for predicting the next 2 days' stock prices using historical data,
    and influencing that prediction with recent news sentiment (VADER, TextBlob, or Llama3).
    The application plots historical prices and extends the graph with the sentiment-adjusted predictions.
    Now includes a base trend for neutral sentiment and multi-day prediction.
    """
    ...
    def __init__(self, master):
        ...
        self.predict_button = tk.Button(self.input_frame, text="Predict Next Day Price & Plot",
    ...
    def _perform_prediction_task(self, ticker, start_date_str, end_date_str, lookback_window,
        ...
        try:
                ...
                for i, item in enumerate(news_items):
                    ...
                    if selected_analyzer == "Llama3":
                        ...
                        try:
                            ...
                            self._update_log_display(f"Warning: Llama3 analysis failed for item {i + 1}: {ollama_e}",
                                                     clear_previous=False)
            ...
            model.add(LSTM(units=50, return_sequences=True,
                           input_shape=(X.shape[1], 1)))  # input_shape: (time_steps, features)
            ...
            # --- Step 5: Predict Raw Next Day Prices (Multi-step prediction) ---
            self._update_status("Predicting next 2 days' raw stock prices...", "blue")

            # Get the last 'lookback_window' days of historical prices for the initial prediction input
            # Make a copy to avoid modifying the original scaled_data
            ...
            for i in range(2):  # Predict for 2 future days
                raw_predicted_scaled_price = model.predict(current_input_sequence, verbose=0)[0, 0]  # Get scalar
                ...
                # Prepare input for the next prediction: remove oldest, add new prediction
                # current_input_sequence is (1, lookback_window, 1)
                # Need to drop the first element and append the new prediction
                # Reshape raw_predicted_scaled_price to (1, 1, 1) before concatenating
                current_input_sequence = np.concatenate((current_input_sequence[:, 1:, :],
                                                         raw_predicted_scaled_price.reshape(1, 1, 1)),
                                                        # Correctly reshape to 3D
                                                        axis=1)

            # --- Step 6: Apply Sentiment-Based Adjustment or Base Trend to Each Prediction ---
            ...
            # Convert to standard datetime objects for safe formatting and plotting
            predicted_dates_for_display = [dt.to_pydatetime() for dt in predicted_dates_ts]

            final_predicted_prices = []
            sentiment_influence_msg = ""  # Message will describe influence for both days
            ...
            sentiment_adjustment_percentage = 0.008  # 0.8% adjustment
            ...
            for raw_predicted_price in raw_predicted_actual_prices:
                ...
                if sentiment_neutral_min_threshold <= latest_sentiment_score <= sentiment_neutral_max_threshold:
                    ...
                    sentiment_influence_msg = f"Strong Positive News ({selected_analyzer} score: {latest_sentiment_score:.2f}). Adjusting predictions upwards by {sentiment_adjustment_percentage * 100:.1f}%."
                    adjusted_price += abs(raw_predicted_price * sentiment_adjustment_percentage)
                elif latest_sentiment_score < sentiment_negative_threshold:
                    sentiment_influence_msg = f"Strong Negative News ({selected_analyzer} score: {latest_sentiment_score:.2f}). Adjusting predictions downwards by {sentiment_adjustment_percentage * 100:.1f}%."
                    adjusted_price -= abs(raw_predicted_price * sentiment_adjustment_percentage)
                else:
                    sentiment_influence_msg = f"Mild News ({selected_analyzer} score: {latest_sentiment_score:.2f}). Predictions rely primarily on LSTM trend."

                final_predicted_prices.append(max(0.01, adjusted_price))  # Ensure price doesn't go below zero
            ...
            self.master.after(0, lambda: messagebox.showinfo("Prediction Complete",
                                                             "Next 2 days stock prices predicted with news sentiment influence!"))
            ...
            self.master.after(0, lambda msg=error_message: self._update_log_display(msg,
                                                                                    clear_previous=False))  # Add to logs
    ...
    def _plot_prediction_and_history(self, ticker, historical_df, predicted_dates, predicted_prices):
        ...
        # Add the predicted points as an extension
        last_historical_date = historical_df.index.max().to_pydatetime()  # Convert to datetime

        # Combine last historical point with predicted points
        all_future_dates = [last_historical_date] + predicted_dates
        all_future_prices = [historical_df['Close'].iloc[-1]] + predicted_prices

        # Plot the connection from last historical to first predicted, and then between predicted points
        self.ax.plot(all_future_dates, all_future_prices, 'ro--', label='Predicted Future Prices',
                     markersize=8)  # Red circle for predictions
