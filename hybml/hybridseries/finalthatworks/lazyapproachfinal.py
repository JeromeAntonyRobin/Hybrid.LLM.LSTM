...
class ForwardLookingStockPredictor:
    ...
    DeepSeek-R1 prompts now include recent price history to make the model more aware of recent trends.
    JSON parsing from DeepSeek-R1 responses has been made more robust to handle conversational filler.
    """
    ...
    def _extract_json_from_ollama_response(self, raw_response, log_prefix=""):
        """
        Extracts a JSON string from a raw Ollama response, handling potential conversational filler.
        Logs the raw response and the parsing attempt.
        """
        self._update_llm_log_display(f"{log_prefix}Raw Response: {raw_response}\n")
        ...
        if json_start != -1 and json_end != -1 and json_end > json_start:
            ...
            try:
                ...
                self._update_llm_log_display(f"{log_prefix}Parsed JSON successfully.\n")
                return parsed_json
            except json.JSONDecodeError as e:
                self._update_llm_log_display(f"{log_prefix}JSON parse failed: {e}. Attempting regex fallback.\n")
                return None  # Indicate failure to parse JSON
        else:
            self._update_llm_log_display(
                f"{log_prefix}No valid JSON object found in raw response. Attempting regex fallback.\n")
            return None  # Indicate no JSON object found
    ...
    def _perform_prediction_task(self, ticker, start_date_str, end_date_str, lookback_window,
        ...
        try:
            ...
            # --- Prepare recent price context for LLM ---
            recent_price_context = ""
            if len(ohlcv_data) >= 10:
                last_10_days = ohlcv_data['Close'].tail(10)
                avg_daily_change = last_10_days.pct_change().mean() * 100 if len(last_10_days) > 1 else 0.0
                max_daily_change = last_10_days.pct_change().abs().max() * 100 if len(last_10_days) > 1 else 0.0
                recent_price_context = (
                    f"Recent 10-day price trend (last close: ${ohlcv_data['Close'].iloc[-1]:.2f}, "
                    f"avg daily change: {avg_daily_change:.2f}%, max daily change: {max_daily_change:.2f}%)."
                )
            elif len(ohlcv_data) > 0:
                recent_price_context = (
                    f"Last close price: ${ohlcv_data['Close'].iloc[-1]:.2f}. "
                    "Not enough data for 10-day trend."
                )
            self._update_log_display(f"Recent Price Context for LLM: {recent_price_context}\n", clear_previous=False)
            ...
            macro_prompt = (
                ...
                f"and the following recent stock price context: {recent_price_context} "
            ...
            try:
                macro_response_raw = ollama.chat(
                    model='deepseek-r1', messages=[{'role': 'user', 'content': macro_prompt}], stream=False,
                    options={'num_predict': 20, 'temperature': 0.001}, format='json'
                )
                macro_raw_answer = macro_response_raw['message']['content'].strip()
                parsed_json_data = self._extract_json_from_ollama_response(macro_raw_answer, "Macro Outlook: ")

                parsed_outlook = "Neutral"  # Default
                if parsed_json_data and 'outlook' in parsed_json_data:
                    parsed_outlook = parsed_json_data['outlook'].strip().capitalize()
                else:
                    # Fallback to regex if JSON parsing fails or outlook key is missing
                ...
                if macro_outlook_classification in ["Bullish", "Bearish"]:
                    ...
                    strength_response_raw = ollama.chat(
                        model='deepseek-r1', messages=[{'role': 'user', 'content': strength_prompt}], stream=False,
                        options={'num_predict': 20, 'temperature': 0.001}, format='json'
                    )
                    strength_raw_answer = strength_response_raw['message']['content'].strip()
                    parsed_json_data = self._extract_json_from_ollama_response(strength_raw_answer, "Macro Strength: ")

                    if parsed_json_data and 'strength' in parsed_json_data:
                        macro_strength = float(parsed_json_data['strength'])
            ...
            fundamental_prompt = (
                ...
                f"Also consider the following recent stock price context: {recent_price_context} "
            ...
            try:
                fundamental_response_raw = ollama.chat(
                    model='deepseek-r1', messages=[{'role': 'user', 'content': fundamental_prompt}], stream=False,
                    options={'num_predict': 20, 'temperature': 0.001}, format='json'
                )
                fundamental_raw_answer = fundamental_response_raw['message']['content'].strip()
                parsed_json_data = self._extract_json_from_ollama_response(fundamental_raw_answer,
                                                                           "Fundamental Outlook: ")
                ...
                if fundamental_outlook_classification in ["Strong", "Weak"]:
                    ...
                    strength_raw_answer = strength_response_raw['message']['content'].strip()
                    parsed_json_data = self._extract_json_from_ollama_response(strength_raw_answer,
                                                                               "Fundamental Strength: ")

                    if parsed_json_data and 'strength' in parsed_json_data:
                        fundamental_strength = float(parsed_json_data['strength'])
            ...
            for i, item in enumerate(news_items):
                ...
                is_relevant = False
                try:
                    relevance_response_raw = ollama.chat(
                        model='deepseek-r1', messages=[{'role': 'user', 'content': relevance_prompt}], stream=False,
                        options={'num_predict': 20, 'temperature': 0.001}, format='json'
                    )
                    relevance_raw_answer = relevance_response_raw['message']['content'].strip()
                    parsed_json_data = self._extract_json_from_ollama_response(relevance_raw_answer, "Relevance: ")

                    parsed_relevance = "NO"
                    if parsed_json_data and 'relevance' in parsed_json_data:
                        parsed_relevance = parsed_json_data['relevance'].strip().upper()
                    else:
                    ...
                    self._update_llm_log_display(
                        f"Parsed Value: '{parsed_relevance}'\n")
            ...
            for i, item in enumerate(relevant_news_items):
                ...
                if selected_analyzer == "DeepSeek-R1":
                    ...
                    try:
                        ollama_response_raw = ollama.chat(
                            model='deepseek-r1', messages=[{'role': 'user', 'content': ollama_prompt}], stream=False,
                            options={'num_predict': 100, 'temperature': 0.001}, format='json'
                        )
                        llm_raw_output = ollama_response_raw['message']['content'].strip()
                        parsed_json_data = self._extract_json_from_ollama_response(llm_raw_output,
                                                                                   "Sentiment/Summary: ")

                        parsed_score = 0.0
                        parsed_summary = ""
                        if parsed_json_data and 'score' in parsed_json_data:
                            parsed_score = float(parsed_json_data['score'])
                        else:
                        ...
                        if parsed_json_data and 'summary' in parsed_json_data:
                            parsed_summary = parsed_json_data['summary'].strip()
                        else:
                ...
                try:
                    risk_response_raw = ollama.chat(
                        model='deepseek-r1', messages=[{'role': 'user', 'content': risk_prompt}], stream=False,
                        options={'num_predict': 50, 'temperature': 0.001}, format='json'
                    )
                    risk_raw_answer = risk_response_raw['message']['content'].strip()
                    parsed_json_data = self._extract_json_from_ollama_response(risk_raw_answer, "Risk Identification: ")

                    parsed_risks_list = []
                    if parsed_json_data and 'risks' in parsed_json_data and isinstance(parsed_json_data['risks'], list):
                        parsed_risks_list = [str(r).strip() for r in parsed_json_data['risks'] if str(r).strip()]
                    else:
                    ...
                    if identified_risks_for_item:  # Only ask for severity if risks were actually identified
                        ...
                        severity_response_raw = ollama.chat(
                            model='deepseek-r1', messages=[{'role': 'user', 'content': severity_prompt}], stream=False,
                            options={'num_predict': 20, 'temperature': 0.001}, format='json'
                        )
                        severity_raw_answer = severity_response_raw['message']['content'].strip()
                        parsed_json_data = self._extract_json_from_ollama_response(severity_raw_answer,
                                                                                   "Risk Severity: ")

                        parsed_severity = 0.0
                        if parsed_json_data and 'severity' in parsed_json_data:
                            parsed_severity = float(parsed_json_data['severity'])
                        else:
            ...
            try:
                market_insight_response_raw = ollama.chat(
                    model='deepseek-r1', messages=[{'role': 'user', 'content': market_insight_prompt}], stream=False,
                    options={'num_predict': 200, 'temperature': 0.001}
                )
                market_insight_raw_answer = market_insight_response_raw['message']['content'].strip()
